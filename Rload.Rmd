...
```{r}
library(rjson)
tweet <- fromJSON(file = "tweet-converter/output.json") # loads 1st tweet, but no more
...
```{r}
ts <- fromJSON(sprintf("[%s]", paste(readLines("data/t1404902603295.json"),collapse=",")), unexpected.escape = "keep")
ts[[2]]$text
